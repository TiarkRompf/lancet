...
trait Core_Str extends Base_Str with Core {
...
trait Base_Opt extends Base {
  ...
  case class Const[+T](x: T) extends Rep[T] { override def toString = x.toString }
  ...
  def reflect[T](s: String): Rep[T] = { val x = fresh; emit("val "+x+" = "+s); Dyn(x) }
...
trait Core_Opt extends Base_Opt with Core {

  implicit def unit(x: Boolean): Rep[Boolean] = Const(x)
  implicit def unit(x: Byte): Rep[Byte] = Const(x)
  implicit def unit(x: Char): Rep[Char] = Const(x)
  implicit def unit(x: Short): Rep[Short] = Const(x)
  implicit def unit(x: Int): Rep[Int] = Const(x)
  implicit def unit(x: Long): Rep[Long] = Const(x)
  implicit def unit(x: Float): Rep[Float] = Const(x)
  implicit def unit(x: Double): Rep[Double] = Const(x)


  def unit(x: Null): Rep[Object] = Const(null)
  def unit(x: Object): Rep[Object] = Const(x)
