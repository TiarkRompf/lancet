...
class FloatSparseMatrixCOO(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Float], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCOO not supported")  
  
  def Clone = { 
    val m = new FloatSparseMatrixCOO(_numRows, _numCols)
    m._data = _data.clone
