...
class IntSparseMatrixCOO(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Int], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCOO not supported")  
  
  def Clone = { 
    val m = new IntSparseMatrixCOO(_numRows, _numCols)
    m._data = _data.clone
