 ...
 */ 
class BooleanSparseMatrixCOO(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Boolean], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCOO not supported")  
  
  def Clone = { 
    val m = new BooleanSparseMatrixCOO(_numRows, _numCols)
    m._data = _data.clone
