 ...
 */ 
class LongSparseMatrixCOO(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Long], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCOO not supported")  
  
  def Clone = { 
    val m = new LongSparseMatrixCOO(_numRows, _numCols)
    m._data = _data.clone
