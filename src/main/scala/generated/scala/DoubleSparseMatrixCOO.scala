 ...
 */ 
class DoubleSparseMatrixCOO(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Double], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCOO not supported")  
  
  def Clone = { 
    val m = new DoubleSparseMatrixCOO(_numRows, _numCols)
    m._data = _data.clone
