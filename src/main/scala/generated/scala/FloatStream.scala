...
class FloatStream(val numRows: Int, val numCols: Int, val chunkSize: Int, val func: (Int,Int) => Float, val isPure: Boolean) { //extends FloatStream {
    val bufRows = math.min(numRows, chunkSize)
    val size = numCols*bufRows
    protected var _data: Array[Float] =  try { new Array[Float](size) }
                                     catch {
    ...
    def chunkRow(idx: Int, offset: Int) = {
      ...
      new FloatStreamRow(idx, offset, this, _data)
    }

    def rawElem(idx: Int): Float = {
      _data(idx)
    }

    def vview(start: Int, stride: Int, length: Int, isRow: Boolean) = {
      new FloatDenseVectorView(_data, start, stride, length, isRow)
    }
}
