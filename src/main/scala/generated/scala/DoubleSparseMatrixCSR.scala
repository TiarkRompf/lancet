...
class DoubleSparseMatrixCSR(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Double], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCSR not supported")  
  
  def Clone = { 
    val m = new DoubleSparseMatrixCSR(_numRows, _numCols)
    m._data = _data.clone
