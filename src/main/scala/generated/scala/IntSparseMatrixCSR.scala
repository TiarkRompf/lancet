 ...
 */
class IntSparseMatrixCSR(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Int], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCSR not supported")  
  
  def Clone = { 
    val m = new IntSparseMatrixCSR(_numRows, _numCols)
    m._data = _data.clone
