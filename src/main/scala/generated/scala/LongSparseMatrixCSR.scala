 ...
 */
class LongSparseMatrixCSR(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Long], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCSR not supported")  
  
  def Clone = { 
    val m = new LongSparseMatrixCSR(_numRows, _numCols)
    m._data = _data.clone
