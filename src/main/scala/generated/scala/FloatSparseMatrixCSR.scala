 ...
 */
class FloatSparseMatrixCSR(__numRows: Int, __numCols: Int) { 
   ...
   */    
  def unsafeSetData(xs: Array[Float], len: Int) = throw new UnsupportedOperationException("unsafeSetData in SparseMatrixCSR not supported")  
  
  def Clone = { 
    val m = new FloatSparseMatrixCSR(_numRows, _numCols)
    m._data = _data.clone
